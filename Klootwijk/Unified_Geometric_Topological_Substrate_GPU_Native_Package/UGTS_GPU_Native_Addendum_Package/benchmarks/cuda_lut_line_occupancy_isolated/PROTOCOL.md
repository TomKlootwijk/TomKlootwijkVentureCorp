...
The dependent pointer-chase result is not a fixed physical cost for independent packed LUT codes. With one useful code per 128-byte region, a 40 MiB allocation retains 99.02% of its 28 MiB texture rate. A universal 128-byte-per-code cache model predicts a transition immediately beyond the reported 36 MiB and is contradicted here.

A simple requested-byte model is also insufficient. At `K=1`, only the first 32-byte part of each region is requested, yet the rate falls to 65.02% at a 48 MiB allocation even though the nominal requested-address span is only 12 MiB. Address tags, translation, set mapping, replacement, scheduling or another undocumented mechanism may contribute. Without performance counters, this experiment cannot distinguish them and makes no physical cache-line or sector claim.
