...
| Does texture add cache capacity? | No measured gain: native texture/global median **0.99949x**, with the same roughly 45.2% 36-to-40 MiB loss | Matched native control |
...
The independent random packed-LUT sweep resolves the space between those endpoints. It shows that even one useful code per aligned 128-byte region does not behave like a fixed 128-byte charge: it retains 99.02% of its 28 MiB rate at a 40 MiB allocation and 65.02% at 48 MiB. Real LUTs therefore fall between the endpoints according to neighborhood reuse, the probability of requesting each packed address region, dependence, and access ordering.
...
Machine-readable paired results are in `windows_physical_gpu_aggregate/cold_lineage_comparison.csv`, `windows_physical_gpu_aggregate/lut_path_comparison.csv`, `windows_physical_gpu_aggregate/l2_latency_comparison.csv`, `windows_physical_gpu_aggregate/cuda_l2_clock_comparison.csv`, `windows_physical_gpu_aggregate/cuda_l2_mlp_comparison.csv`, `windows_physical_gpu_aggregate/cuda_texture_lut_comparison.csv`, `windows_physical_gpu_aggregate/cuda_l2_stride_comparison.csv`, and the `cuda_packed_log_lut_*.csv` tables; the line-occupancy aggregate and CSV tables are under `cuda_lut_line_occupancy_isolated/aggregate/`. Compiler metadata is in `windows_physical_gpu_aggregate/pipeline_executable_statistics.csv`, and the earlier combined aggregate is `windows_physical_gpu_aggregate/aggregate_metrics.json`. Vulkan shader-clock raw data is under `l2_latency_isolated/`; CUDA cycle, concurrency, texture-path, dense-packing, sparse-residency and line-occupancy data are under `cuda_l2_clock_isolated/`, `cuda_l2_mlp_isolated/`, `cuda_texture_lut_isolated/`, `cuda_packed_log_lut_isolated/`, `cuda_l2_stride_isolated/` and `cuda_lut_line_occupancy_isolated/`; cache-path controls are under `lut_path_control/`; cold-lineage primary results are under `cold_lineage_isolated/`; `native_capability_probe/` records the selected-device extension evidence. The earlier overlapping-process `cold_lineage_hot/` corpus is retained but excluded from primary claims.
