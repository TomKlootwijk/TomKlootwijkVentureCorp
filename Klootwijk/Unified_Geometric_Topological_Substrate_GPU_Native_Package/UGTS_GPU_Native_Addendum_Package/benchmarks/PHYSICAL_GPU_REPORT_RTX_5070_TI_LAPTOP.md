...
A sparse-stride CUDA control now bounds how locality changes that ceiling. Full-occupancy active-node capacity scales 4:2:1 for 32-, 64- and 128-byte spacing, and 256-byte spacing tracks the 128-byte curve despite twice the allocation. This is consistent with a 128-byte effective residency unit per isolated random `u32` on this workload. The nominal 36 MiB therefore spans 50,331,648 densely packed 6-bit codes but only 294,912 isolated active entries, a 170.667x locality range. This is an end-to-end residency inference, not a physical-line or counter claim.
...
Dense packed6 therefore spans 170.667x more useful codes than the isolated-access extreme at the same cache budget. A real LUT lies between those bounds according to spatial reuse; packing cannot rescue a data structure that touches only one useful word per effective cache region.
