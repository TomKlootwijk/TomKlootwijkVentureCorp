...
if (-not (Test-Path -LiteralPath $nvcc)) {
    throw "CUDA compiler not found: $nvcc"
}
...
$source = Join-Path $gpuRoot 'src\ugts_cuda_packed_log_lut_bench.cu'
$executable = Join-Path $outputPath 'ugts_cuda_packed_log_lut_bench.exe'

& $nvcc -std=c++17 -O3 -lineinfo -arch=sm_120 -Xptxas=-v `
    $source -o $executable
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $executable)) {
    throw "CUDA packed log-LUT benchmark build failed with exit code $LASTEXITCODE"
}
