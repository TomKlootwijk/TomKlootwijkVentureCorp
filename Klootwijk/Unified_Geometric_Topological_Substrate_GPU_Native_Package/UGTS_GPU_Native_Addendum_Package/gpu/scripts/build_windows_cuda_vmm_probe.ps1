...
$source = Join-Path $gpuRoot 'src\ugts_cuda_vmm_granularity_probe.cpp'
$executable = Join-Path $outputPath 'ugts_cuda_vmm_granularity_probe.exe'
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $executable)) {
    throw "CUDA VMM granularity probe build failed with exit code $LASTEXITCODE"
}
