...
$source = Join-Path $gpuRoot 'src\ugts_cuda_lut_sparse_address_bench.cu'
$executable = Join-Path $outputPath 'ugts_cuda_lut_sparse_address_bench.exe'
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $executable)) {
    throw "CUDA sparse-address LUT benchmark build failed with exit code $LASTEXITCODE"
}
