...
$source = Join-Path $gpuRoot 'src\ugts_cuda_vmm_alias_bench.cu'
$executable = Join-Path $outputPath 'ugts_cuda_vmm_alias_bench.exe'
& $compiler -std=c++17 -O3 -lineinfo -arch=sm_120 -I (Join-Path $CudaRoot 'include') $source `
    -L (Join-Path $CudaRoot 'lib\x64') -lcuda -o $executable
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $executable)) {
    throw "CUDA VMM-alias benchmark build failed with exit code $LASTEXITCODE"
}
