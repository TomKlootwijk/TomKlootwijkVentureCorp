...
$source = Join-Path $gpuRoot 'src\ugts_cuda_lut_line_occupancy_bench.cu'
$executable = Join-Path $outputPath 'ugts_cuda_lut_line_occupancy_bench.exe'
...
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $executable)) {
    throw "CUDA LUT line-occupancy benchmark build failed with exit code $LASTEXITCODE"
}
