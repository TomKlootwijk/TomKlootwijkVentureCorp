...
$source = Join-Path $gpuRoot 'src\ugts_cuda_texture_lut_bench.cu'
$executable = Join-Path $outputPath 'ugts_cuda_texture_lut_bench.exe'

& $nvcc -std=c++17 -O3 -lineinfo -arch=sm_120 -Xptxas=-v `
    $source -o $executable
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $executable)) {
    throw "CUDA texture-LUT benchmark build failed with exit code $LASTEXITCODE"
}
