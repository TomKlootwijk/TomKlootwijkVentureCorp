...
std::vector<SortedPoint> morton_sort(const std::vector<Vec3>& points) {
    Vec3 minp{
...
PackedData pack_points(
    ...
    const std::vector<SortedPoint> sorted = morton_sort(points);
    ...
    packed.header.flags = kFlagXorSwizzle | kFlagMortonSorted | kFlagEvenParity | kFlagLogSpherical;
